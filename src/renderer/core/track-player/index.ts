...
class TrackPlayer {
    get currentMusic() { return currentMusicStore.getValue(); }
    ...
    get progress() { return progressStore.getValue(); }
    get playerState() { return playerStateStore.getValue(); }
    get repeatMode() { return repeatModeStore.getValue(); }
    get currentQuality() { return currentQualityStore.getValue(); }
    get speed() { return currentSpeedStore.getValue(); }
    get volume() { return currentVolumeStore.getValue(); }
    get lyric() { return currentLyricStore.getValue(); }
    get musicQueue() { return musicQueueStore.getValue(); }
    get isEmpty() { return this.musicQueue.length <= 0; }
    ...
    public on<T extends keyof InternalPlayerEvents>(event: T, callback: InternalPlayerEvents[T]) {
    ...
    public async setup() {
        ...
        messageBus.onCommand('mpvFinished', () => this.handlePlaybackFinished());

        // 监听主进程通过 appStatePatch 命令同步过来的状态
    ...
    public async playIndex(index: number, options: IPlayOptions = {}) {
        ...
        if (index === -1 && this.musicQueue.length === 0) return;
        ...
        if (this.currentIndex === index && this.isCurrentMusic(this.musicQueue[index]) && !refreshSource) {
            if (restartOnSameMedia) this.seekTo(0);
        ...
        try {
            ...
            if (!mediaSource?.url) throw new Error("mediaSource.url is empty");
            if (!this.isCurrentMusic(nextMusicItem)) return;

            this.setCurrentQuality(quality);
            this.setTrack(mediaSource, nextMusicItem, { seekTo, autoPlay: true });
    ...
    public async playMusicWithReplaceQueue(musicList: IMusic.IMusicItem[], musicItem?: IMusic.IMusicItem) {
        if (!musicList.length && !musicItem) return;
    ...
    public skipToPrev() {
        if (this.isEmpty) { this.setCurrentMusic(null); this.currentIndex = -1; return; }
    ...
    public seekTo(seconds: number) { messageBus.sendCommand('mpvSeek', seconds); }
    public pause() {
        if (this.playerState === PlayerState.Playing) {
            messageBus.sendCommand('mpvTogglePause');
    ...
    public async resume() {
        ...
        if (this.playerState === PlayerState.Paused) {
            ...
            await this.playIndex(this.currentIndex, { restartOnSameMedia: false, seekTo: this.progress.currentTime });
        }
    }
    ...
    public setVolume(volume: number, sendCommand = true) {
        ...
        if (sendCommand) messageBus.sendCommand('mpvSetVolume', volume);
    }

    public setSpeed(speed: number, sendCommand = true) {
        currentSpeedStore.setValue(speed);
        setUserPreference("speed", speed);
        // 注意：此处需要一个 mpvSetSpeed 命令，我们暂未在 type.d.ts 定义，需要添加
        // if (sendCommand) messageBus.sendCommand('mpvSetSpeed', speed);
    }
    ...
    public addNext(musicItems: IMusic.IMusicItem | IMusic.IMusicItem[]) {
        ...
        const itemsWithTimestamp = _musicItems.map((item, index) => {
            if (duplicateIndex === -1 && this.isCurrentMusic(item)) duplicateIndex = index;
            return { ...item, [timeStampSymbol]: now, [sortIndexSymbol]: index };
        });

        if (duplicateIndex !== -1) {
            const dupItem = itemsWithTimestamp.splice(duplicateIndex, 1);
            itemsWithTimestamp.unshift(dupItem[0]);
        }

        const startPart = [];
        const tailPart = [];
        const oldQueue = this.musicQueue;
        const uniqueMap = createUniqueMap(itemsWithTimestamp);

        for (let i = 0; i < oldQueue.length; ++i) {
            if (i <= this.currentIndex) {
                if (!uniqueMap.has(oldQueue[i])) startPart.push(oldQueue[i]);
            } else {
                if (!uniqueMap.has(oldQueue[i])) tailPart.push(oldQueue[i]);
            }
        }
        const newQueue = [...startPart, ...itemsWithTimestamp, ...tailPart];
        this.setMusicQueue(newQueue);
    }

    public removeMusic(musicItems: IMusic.IMusicItem | IMusic.IMusicItem[] | number) {
        const musicIndex = typeof musicItems === "number" ? musicItems : this.findMusicIndex(musicItems as IMusic.IMusicItem);
        if (musicIndex !== -1) {
            ...
            this.setMusicQueue(newQueue);
        } else if (Array.isArray(musicItems)) {
            ...
            for (let i = 0; i < oldQueue.length; i++) {
                if (uniqueMap.has(oldQueue[i])) {
                    ...
                    if (this.currentIndex === i) this.currentIndex = newQueue.length - 1;
                }
            }
            this.setMusicQueue(newQueue);
        }
    ...
    public setRepeatMode(repeatMode: RepeatMode, sendCommand = true) {
        ...
        // [新增] 将循环状态同步到主进程
        if (sendCommand) {
            messageBus.sendCommand('mpvSetLoop', repeatMode === RepeatMode.Loop);
        }
    ...
    public async fetchCurrentLyric(forceLoad = false) {
        ...
        if (!forceLoad && this.lyric && this.isCurrentMusic(this.lyric.parser?.musicItem)) return;

        try {
            const linkedLyricItem = await getLinkedLyric(currentMusic);
            let lyricSource: ILyric.ILyricSource;
            if (linkedLyricItem) {
                lyricSource = await PluginManager.callPluginDelegateMethod(linkedLyricItem, "getLyric", linkedLyricItem);
            }
            if (!lyricSource && this.isCurrentMusic(currentMusic)) {
                lyricSource = await PluginManager.callPluginDelegateMethod(currentMusic, "getLyric", currentMusic);
            }
    ...
    private async fetchMediaSource(musicItem: IMusic.IMusicItem, quality?: IMusic.IQualityKey) {
        ...
        if (downloadedData) {
            ...
            if (await fsUtil.isFile(_path)) {
                return { quality, mediaSource: { url: fsUtil.addFileScheme(_path) } };
            }
        }
        ...
        for (const q of qualityOrder) {
            try {
                ...
                if (mediaSource?.url) { realQuality = q; break; }
            } catch {}
        }
    ...
    private setCurrentMusic(musicItem: IMusic.IMusicItem | null) {
        if (!isSameMedia(musicItem, this.currentMusic)) {
